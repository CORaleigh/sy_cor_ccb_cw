...
public class HandleCCBFADataInterfaceBean_Impl implements
	...
	public String getMessage(String messagebody) {
		...
		try {
		    ...
		    System.out.println("registers " + request.getRegisters());
		    ...
		    // readSequence is not passed over and will be null on new start services
		    //System.out.println("RegisterSeq" + request.getRegisters().getReadSequence());
			System.out.println("Register1");
		    System.out.println("Reading " + request.getRegisters().get(0).getLastMeterRead().toBigInteger());
		    System.out.println("Dials " +  request.getRegisters().get(0).getDials());
		    System.out.println("Miu " + request.getRegisters().get(0).getMIU());
		    System.out.println("ReadType " + request.getRegisters().get(0).getReadType());
		    System.out.println("MrSource " + request.getRegisters().get(0).getMrSource());
		    System.out.println("Size " + request.getRegisters().get(0).getRegisterSize()); 
		    
		    if (externalID  == null) {
		    System.out.println("LowReadThreshold " + request.getRegisters().get(0).getLowMeterReadingWarning().toBigInteger());
		    System.out.println("HighReadThreshold " + request.getRegisters().get(0).getHighMeterReadingWarning().toBigInteger());
		    };
		    ...
		    System.out.println("registers " + request.getYCoordinate());
			...
			if (externalID == null && !compoundMeter) {
			System.out.println("This is an Insert and is not a compound meter");	
			Map config = new HashMap();
			JsonBuilderFactory factory = Json.createBuilderFactory(config);
			String meterHeader = "MeterHeader";
			JsonObject value = factory.createObjectBuilder()
					.add(meterHeader,  factory.createObjectBuilder()
				     ...
				     .add("WorkOrder", factory.createObjectBuilder()
				    		 ...
				    		 // remove meter = 257700
				    		 // install meter = 257701
				    		 // service existing meter = 257702
				    		 .add("WoTemplateId", "257702")
				    		 .add("WoAddress", request.getServiceAddress())
				    		 .add("WoXCoordinate", request.getXCoordinate())
				    		 .add("WoYCoordinate", request.getYCoordinate()))
					...
					.add(meterHeader,  factory.createObjectBuilder()
				     ...
				     .add("CurrentMeter", factory.createObjectBuilder()
					     ...
					     .add("Size", factory.createObjectBuilder().add("Code", request.getMeterSize()))
				    	     ...
				    	     .add("LowReadThreshold", request.getRegisters().get(1).getLowMeterReadingWarning().toBigInteger())
				    		 .add("HighReadThreshold", request.getRegisters().get(1).getHighMeterReadingWarning().toBigInteger()))
				    	 //.add("DeviceTest", "null")) // must be a true null
				    	 	.addNull("DeviceTest"))
				     //.add("InstallMeter", "null")
				     .addNull("InstallMeter")
